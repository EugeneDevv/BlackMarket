package com.panaceasoft.riftblackmarket.repository.chat;
...
public class ChatRepository extends PSRepository {
     ...
     */
    @Inject
    public ChatRepository(PSApiService psApiService, AppExecutors appExecutors, PSCoreDb db) {
...
//                        }
    ...
    public LiveData<Resource<Boolean>> uploadActiveStateToFirebase(UserStatusHolder userStatusHolder) {
        ...
        appExecutors.networkIO().execute(() -> {
            ...
            databaseReference.child(Config.USER_PRESENCE).child(userStatusHolder.userId).onDisconnect().removeValue().addOnCompleteListener(task -> statusLiveData.postValue(Resource.successWithMsg(Config.SUCCESSFULLY_DELTED, true)));
...
//    }
    ...
    public LiveData<Resource<Boolean>> uploadChattingWith(Chat chat) {
        ...
        appExecutors.networkIO().execute(() -> {
            ...
            databaseReference.child(Config.CHAT_WITH).child(chat.getSender_id()).onDisconnect().removeValue().addOnCompleteListener(task -> statusLiveData.postValue(Resource.successWithMsg(Config.SUCCESSFULLY_DELTED, true)));
